...
@include('home.navbar')
    ...
    <!-- Search Box -->
    <div class="w-full flex justify-end my-4 px-4 md:px-6">
      <input
        type="text"
        id="searchBox"
        placeholder="Search by country..."
        class="max-w-xl px-4 py-2 border border-borderDefault rounded-lg focus:ring-2 focus:ring-primary outline-none"
      />
    </div>
    ...
    <!-- Contact List -->
    <section id="contactList" class="w-full mx-auto p-4 xl:grid grid-cols-2 items-baseline gap-5 space-y-3"></section>
    ...
    <script>
      ...
      function renderContacts(filter = "") {
        container.innerHTML = "";
        contacts
          .filter((c) => c.country.toLowerCase().includes(filter.toLowerCase()))
          .forEach((c) => {
            const card = document.createElement("div");
            card.className =
              "bg-bgPrimary rounded-sm p-6 border border-borderDefault";
            card.innerHTML = `
            <h2 class="text-xl font-semibold text-textPrimary mb-4">${c.country}</h2>
